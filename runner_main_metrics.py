...
def main():
    ...
    main_metrics_df["flesch_reading_ease"] = bacot_df["answer"].apply(MainMetrics.evaluate_flesch_reading_ease)
    main_metrics_df["flesch_kincaid_grade"] = bacot_df["answer"].apply(MainMetrics.evaluate_flesch_kincaid_grade)
    
    logger.info("Evaluating Relevance...")
    bert_eval_dict = MainMetrics.evaluate_bert_score(candidate=bacot_df["answer"], reference=bacot_df["human_answer"])
    ...
    main_metrics_df["recommendations_precision_50"] = bacot_df.apply(lambda x: MainMetrics.evaluate_actionability(recommendations_to_check=x["rec_ai_list"], recommendations_reference=x["rec_human_str"]), axis=1)

    logging.info("Evaluating Reasoning...")
    main_metrics_df["reasoning_percentage"] = bacot_df["reasoning_dict"].apply(lambda x: MainMetrics.evaluate_reasoning)
