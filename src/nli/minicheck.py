...
class NLIMiniCheckEvaluator:
    ...
    def label_claim_minicheck(
        ...
        if isinstance(nli_model, BespokeLabs):
            for prem in premises:
                resp = nli_model.minicheck.factcheck.create(claim=claim, context=prem)
    ...
    def evaluate_answer_minicheck(
        ...
        for c in claims_to_check:
            try:
                ...
                print(claims_to_check)
            try:
                best_label_list, best_prob_list, best_evidence_list = (
                    NLIMiniCheckEvaluator.label_claim_minicheck(
                        nli_model, c, [context_document], k=k
                    )
                )
            except:
    ...
    def calculate_factuality_precision(fact_check_dict: dict, prefix: str):
        try:
            return (
                1
                - fact_check_dict[f"{prefix}_summary"]["NOT_ALIGN"]
                / fact_check_dict[f"{prefix}_summary"]["total"]
            )
        except:
            return (
                fact_check_dict[f"{prefix}_summary"]["ALIGN"]
                / fact_check_dict[f"{prefix}_summary"]["total"]
            )
